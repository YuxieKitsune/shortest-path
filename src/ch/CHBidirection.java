...
public class CHBidirection {
	...
	public SearchPair search(List<List<Node>> adj, List<List<Node>> adjR, int[] priority, int src, int srcR) {
		this.adj = adj;
		this.adjR = adjR;
...
//		System.out.println("Distance: " + mu);
	...
	private int expNeighbours(int currentNode, int mu) {
		...
		for (int i = 1; i < adj.get(currentNode).size(); i++) {
			...
			if (priority[neighbourNode.nodeid] > priority[currentNode] && !settled.contains(neighbourNode.nodeid)) {
	...
	private int expNeighboursR(int currentNodeR, int mu) {
		...
		for (int i = 1; i < adjR.get(currentNodeR).size(); i++) {
			...
			if (priority[neighbourNode.nodeid] > priority[currentNodeR] && !settledR.contains(neighbourNode.nodeid)) {
				edgeDist = neighbourNode.cost;
